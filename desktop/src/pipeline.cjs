...
class Pipeline{
 ...
 async create(user,input){if(this.running)throw Error('已有任务运行中');if(!['image','video','saved'].includes(input.person)||!['clone','audio','saved'].includes(input.voice)||input.consent!==true)throw Error('请确认素材使用授权');if(input.voice!=='audio'&&(!input.script?.trim()||input.script.length>20000))throw Error('请填写文案（不超过20000字）');
