...
- Voice only: reference voice + script → WAV. Run `python scripts/run_pipeline.py --voice-only --voice reference.wav --script-file script.txt --output cloned.wav`.
- Avatar only: portrait + existing WAV/MP3 → raw digital-human MP4. Run `python scripts/run_pipeline.py --avatar-only --image person.png --audio speech.wav --output avatar.mp4`.
- Package only: raw digital-human MP4 + script → HyperFrames mouthpiece MP4. Run `python scripts/render_mouthpiece.py --video avatar.mp4 --script-file script.txt --output final.mp4`.
- Complete pipeline: portrait + reference voice + script → cloned speech → avatar → packaged MP4. Use the command below.
