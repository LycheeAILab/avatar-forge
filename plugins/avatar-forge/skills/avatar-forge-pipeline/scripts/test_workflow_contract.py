...
require('data["speakerId"] = str(data.get("speakerId") or data["requestId"]).strip()' in SCRIPT, "Clone request_id must automatically become speaker_id")
require("voice is still preparing; retrying" in SCRIPT, "Voice readiness retry is missing")
require("Obtain it from the platform" not in SCRIPT, "The skill must not require manual speaker_id retrieval")
